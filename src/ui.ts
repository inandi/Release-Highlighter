...
export class Ui {
  ...
  constructor(
    ...
    this.classPrefix =
      classPrefix && classPrefix.trim().length > 0 ? classPrefix : null;
    this.root = el("div", classes("rh-root", pref(this.classPrefix, "root")));
    applyTheme(this.root, theme);

    this.overlay = el(
      "div",
      classes("rh-overlay", pref(this.classPrefix, "overlay")),
    );
    ...
    this.highlight = el(
      "div",
      classes("rh-highlight", pref(this.classPrefix, "highlight")),
    );
    this.tooltip = el(
      "div",
      classes("rh-tooltip", pref(this.classPrefix, "tooltip")),
    );
    this.arrow = el(
      "div",
      classes("rh-arrow", pref(this.classPrefix, "arrow")),
    );
  ...
  render(ctx: RenderContext): void {
    this.overlay.style.pointerEvents = ctx.closeOnOverlayClick
      ? "auto"
      : "none";
  ...
  private renderContent(ctx: RenderContext): void {
    ...
    if (step.title) {
      const header = el(
        "div",
        classes("rh-header", pref(this.classPrefix, "header")),
      );
    ...
    if (step.body) {
      const body = el(
        "div",
        classes("rh-body", pref(this.classPrefix, "body")),
      );
    ...
    const controls = el(
      "div",
      classes("rh-controls", pref(this.classPrefix, "controls")),
    );
    const stepText = el(
      "div",
      classes("rh-step", pref(this.classPrefix, "step")),
    );
    ...
    if (ctx.index > 0) {
      controls.appendChild(
        this.button(
          labels.prev,
          classes("rh-btn-ghost", pref(this.classPrefix, "btn-ghost")),
